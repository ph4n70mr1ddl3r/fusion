...
- [ ] T092 [US2] Implement `ApInvoiceService` (create with line items, auto-calculate totals, auto-generate invoice number via `crates/document_numbering` with document type `AP_INV` (distinct from AR's `AR_INV` to maintain separate sequences per FR-044), approve, post with GL journal entry via NATS saga; NOTE: 3-way match for PO-linked invoices is added in T182b (Phase 8) after procurement service exists; currency validation uses `crates/currency::ExchangeRateClient` — single-currency no-op in v1, multi-currency via consolidation service in Phase 11) in `services/ap/src/service/invoice_service.rs`
