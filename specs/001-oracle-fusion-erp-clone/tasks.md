...
- [ ] T059d [P] Create setup wizard database migrations for `legal_entities`, `fiscal_calendars`, `chart_of_accounts_templates` tables in `services/gl/migrations/` — these are tenant-scoped tables populated during wizard
- [ ] T059e [P] Implement `SetupWizardService` gRPC handlers in `services/identity/src/handlers/setup_handler.rs` — `CreateLegalEntity` (name + base currency → consolidation service), `ImportCoATemplate` (industry/region selection → bulk-insert accounts), `ConfigureFiscalCalendar` (year start, periods), `ImportOpeningBalances` (CSV upload: account code + balance + cutoff date → creates opening journal entry)
...
- [ ] T092 [US2] Implement `ApInvoiceService` (create with line items, auto-calculate totals, auto-generate invoice number via `crates/document_numbering` with prefix `INV`, approve, post with GL journal entry via NATS saga; NOTE: 3-way match for PO-linked invoices is added in T182b (Phase 8) after procurement service exists; currency validation uses `crates/currency::ExchangeRateClient` — single-currency no-op in v1, multi-currency via consolidation service in Phase 11) in `services/ap/src/service/invoice_service.rs`
