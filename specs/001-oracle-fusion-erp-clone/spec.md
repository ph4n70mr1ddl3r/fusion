...
- **FR-033**: System MUST enforce strong password policies (minimum 12 characters, at least one uppercase letter, one digit, one special character) and support session management with configurable idle timeouts (default 30 minutes, configurable per tenant via the `tenant_settings` table in the identity service). Idle timeout is enforced at the gateway middleware layer based on last-activity timestamp in the JWT claims and is distinct from the JWT access token TTL (managed by FR-033a).
