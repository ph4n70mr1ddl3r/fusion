...
- **FR-021**: System MUST provide a financial dashboard with key performance indicators (revenue, expenses, cash position, receivables, payables). KPIs MUST reflect data no older than 60 seconds at time of display (achieved via backend moka cache with 55s TTL, event-driven cache invalidation on data changes, and 5-second frontend polling). A manual refresh option MUST also be provided.
...
- **FR-033**: System MUST enforce strong password policies (minimum 12 characters, at least one uppercase letter, one digit, one special character) and support session management with configurable idle timeouts (default 30 minutes, configurable per tenant). Idle timeout is enforced at the gateway middleware layer based on last-activity timestamp in the JWT claims and is distinct from the JWT access token TTL (managed by FR-033a).
...
- **Document Numbering**: Business documents (invoices, journal entries, purchase orders, payments, requisitions, etc.) use auto-generated sequential numbering per tenant with configurable prefixes per document type (e.g., `AP_INV-2026-0001` for payables, `AR_INV-2026-0001` for receivables, `JE-2026-0001` for journal entries, `PO-2026-0001` for purchase orders). The sequence is per-tenant, per document type, per year. Gaps in sequences from voided or deleted drafts are acceptable.
