...
- **FR-044**: System MUST auto-generate sequential document numbers for all business documents (journal entries, invoices, purchase orders, payments, requisitions, budgets) per tenant with configurable prefixes per document type (e.g., `AP_INV-2026-0001` for payables, `AR_INV-2026-0001` for receivables, `JE-2026-0001` for journal entries). Sequences reset at the start of each calendar year (January 1). Gaps from voided or deleted drafts are acceptable and do not require backfill.
...
- **FR-012**: System MUST support invoice creation, distribution, and posting with automatic revenue recognition in the GL. In v1, revenue is recognized upon invoice posting (accrual basis, simple model); deferred revenue scheduling is a future enhancement.
...
- **FR-021**: System MUST provide a financial dashboard with key performance indicators (revenue, expenses, cash position, receivables, payables). KPIs MUST reflect data no older than 60 seconds at time of display. Achieved via: (1) per-tenant backend moka cache with 55s TTL, (2) NATS event-driven cache invalidation on `JournalEntryPosted`/`PaymentProcessed`/`ReceiptProcessed` events, (3) frontend polling at 5-second intervals. Worst-case staleness = 55s backend cache + 5s frontend poll = 60s. A manual refresh option MUST also be provided.
- **FR-022**: System MUST support a report builder tool for creating custom reports with configurable fields, filters, and groupings. The report builder MUST support querying data from GL account balances and transactions, AP invoices and payments, AR invoices and receipts, and budget actuals. Custom report definitions are saved and re-runnable. Cross-service data aggregation is performed via sequential gRPC calls to each source service (GL, AP, AR, budget); if any source service is unavailable, the report returns partial results with a warning indicating which service data is missing. Supported aggregation functions for v1: SUM, COUNT, AVERAGE grouped by any selected field.
- **FR-023**: System MUST support exporting reports in common formats (PDF, spreadsheet/CSV).
...
- **FR-028**: System MUST support transaction entry and reporting in multiple currencies with manually maintained exchange rates. Administrators enter rates via a currency management UI specifying currency pair, rate, and effective date. Automated exchange rate feeds are deferred to a future enhancement. Stale-rate warnings (EC-2) apply to AP/AR invoice entry in v1; GL manual entries and consolidation revaluation assume administrator-validated rates.
...
- **FR-033**: System MUST enforce strong password policies (minimum 12 characters, at least one uppercase letter, one digit, one special character) and support session management with configurable idle timeouts (default 30 minutes, configurable per tenant via the `tenant_settings` table in the identity service). Idle timeout is enforced at the gateway middleware layer via a server-side activity tracker (per-tenant moka cache mapping `user_id → last_activity_timestamp`). On each authenticated request, the gateway checks if `now − last_activity > configured timeout`; if expired, the request is rejected with HTTP 401 and the user must re-authenticate. Otherwise, the timestamp is refreshed. This mechanism is distinct from the JWT access token TTL (managed by FR-033a).
...
- **FR-035**: System MUST send in-app notifications to approvers when transactions require their action. Notifications are delivered via a polling-based mechanism with a polling interval of 30 seconds, a notifications page, and bell icon badge showing unread count. General notification email delivery (approval alerts, system notifications) is deferred to a future enhancement. Transactional/auth emails (password reset links per FR-033a) are included in v1 and delivered via the notification service's SMTP transport.
- **FR-036**: System MUST support approval delegation and escalation with configurable timeout periods (default: 24 hours, valid range: 1–168 hours / 1 hour to 7 days).
...
- **FR-041**: System MUST calculate and post depreciation expense automatically at period end using configurable depreciation methods (straight-line, declining balance). Declining balance defaults to double-declining (200% of straight-line rate); the factor is configurable per asset at creation time.
...
- **Payment**: A disbursement (AP) or receipt (AR) of funds. Key attributes include payment number, date, method (enum: `CHECK`, `WIRE_TRANSFER`, `ACH`, `CASH`), bank account, amount, linked invoices, and reconciliation status.
...
- **Document Numbering**: Auto-generated sequential numbering per tenant per document type per year, with configurable prefixes. See FR-044 for full specification including format and gap policy.
...
- **SC-005**: The approval workflow routes and persists notifications to the approver's inbox within 5 seconds of transaction submission (backend SLA: from transaction submit to notification record persisted and retrievable via API). User-visible display latency is bounded by the client polling interval (30 seconds for in-app notifications per FR-035).
...
- Q: How should business documents (invoices, journal entries, purchase orders) be numbered? → A: Auto-generated sequential numbering per tenant with configurable prefixes per document type (e.g., `AP_INV-2026-0001` for payables, `AR_INV-2026-0001` for receivables, `JE-2026-0001` for journal entries).
